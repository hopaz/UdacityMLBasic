...
# ### 问题 1 - 回答：RM越大，MDEV越大，因为房间越多，房子总面积越大，房屋总价越高。
...
# ### 问题 3 - 回答: 能成功描述目标变量的变化，评分非常接近于1了，模型的准确性很好。
...
# ### 问题 4 - 回答: 图4中最大深度是10。随着训练数据的增加，训练集曲线的评分略有下降，验证集曲线趋于平稳。更多的数据不能提升模型的表现，因为数据量更大，但是图中的验证集曲线的评分没有升高。
...
# ### 问题 5 - 回答: 模型最大深度是1时，模型的预测出现了很大的偏差，因为评分低。最大深度是10时，模型的预测出现较大的方差，因为图形的不确定范围变大。

# ### 问题 6- 最优模型的猜测
# *结合问题 5 中的图，你认为最大深度是多少的模型能够最好地对未见过的数据进行预测？你得出这个答案的依据是什么？*

# ### 问题 6 - 回答: 最大深度是6时模型表现较好。因为此时训练集得分较高，同时验证集方差也适中。
...
# ### 问题 9 - 回答：
# 算法得出的最佳深度是4，我在问题6中得到的最大深度是6，二者不同。
...
# ### 问题 10 - 回答：
# 客户一，推荐价格为$411,096.00。客户二，推荐价格为$219,961.54。客户三，推荐价格为$955,500.00。价格相对来说是合理的，因为随着房间数增加，贫困指数降低即人们大多数有能力担负起房租，学生老师比降低即师资充裕，房假应该越贵。
...
# ### 问题 12 - 回答：
# 很健壮，运行完可以看到这10次的预测结果差异很小，都在400万左右。
...
# ### 问题 13 - 回答：
# 不太适用。通货膨胀导致原本市值为100w的房子，现在价值500w,两者差异悬殊。
# 
